...
In the command line, enter ```sh ssh root@arduino\.local``` (or ```sh ssh root@yourarduinoname\.local``` if the name of the Yun was changed in the aforementioned step), followed by Yes to add the Yun as a known host. The program will ask for a password, in which one would pass in the credentials found earlier on the Yun’s configure page. Once successfully SSHed, the user is now in a position to run the necessary updates in order to ensure the projects’ successful operation. These commands can be found as follows:
